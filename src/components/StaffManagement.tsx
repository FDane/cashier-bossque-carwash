    ...
    if (window.confirm(t('staff.clockOutConfirm' as any) || 'Clock out selected staff?')) {
      try {
        ...
        try {
          ...
          if (statsSnap.exists()) {
            totalCars = statsSnap.data().totalCars || 0;
          }
        } catch (statsErr) {
        ...
        // Create a clean list of just the names
        const staffListText = selectedRecords.map(r => {
          const name = staffMap[r.staffId]?.name || staffMap[r.staffId]?.displayName || r.staffId;
          return `▫️ ${name}`;
        }).join('\n');

        // Put the shared total cars cleanly at the bottom
        const messageText = `🚗 *Carwash Bossque*\n\nAdmin telah mendaftar keluar (Clock-Out) pekerja.\n\n👥 *Senarai Pekerja:*\n${staffListText}\n\n🚘 *Jumlah Kereta (Hari Ini):* ${totalCars} buah\n⏰ *Masa:* ${timeStr}`;
        // 4. Send the notification securely via your API route
        fetch('/api/send-whatsapp', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ message: messageText })
        }).catch(err => console.error('Gagal menghantar notifikasi WhatsApp:', err));
